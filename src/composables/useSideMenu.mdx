...
To use the `useSideMenu` composable, first import it and then invoke it within your setup function. You can optionally pass an initial `activeItemKey`, an object of `expandedKeys`, and the `items` array that represents your menu structure.
...
const {activeItemKey, setActiveItemKey} = useSideMenu(
	'menuItem1_1',
	{
		menuItem1: true,
		menuItem2: true,
...
Finds and returns an item object from the items array based on the provided key. This function is useful when you need to retrieve the full item object, including nested items, by its key.
...
const item = getItemByKey('menuItem2_1'); // will return {label: 'Item 2.1', key: 'menuItem2_1'}
```
