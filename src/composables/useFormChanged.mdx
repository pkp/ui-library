...
Pass the option `warnOnClose: true`. This will automatically attach a `beforeunload` listener for browser tabs and register the close callback for modals.
...
To close the modal when using a form’s Cancel button, make sure to attach the `@cancel="closeModal"` event:
