...
The component manages its own selection state internally. Use the `select` event to react to changes.

```vue
<PkpCombobox :items="countries" @select="onSelect" />
```

### Uncontrolled with default value

Pre-select an item on mount using `defaultValue`. The component still manages state internally after that.

```vue
<PkpCombobox :items="countries" :default-value="countries[0]" />
```

### Controlled (v-model)

Bind selection state externally via `v-model`. The parent owns the state and can programmatically set, reset, or sync it.

```vue
<PkpCombobox :items="countries" v-model="selected" />
```
