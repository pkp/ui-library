...
export const usePkpOpenReviewsStore = defineStore('PkpOpenReviews', () => {
	// State
	...
	function initialize({
		...
		// The store is shared between PkpOpenReviewsSummary (article tab) and
		// PkpOpenReviews (peer-review-record tab), both of which call initialize()
	...
	const reviewerGroups = computed(() => {
		...
		return Array.from(reviewerMap.values()).sort((a, b) => {
			if (!a?.reviewerFullName) {
				return -1;
			} else if (!b?.reviewerFullName) {
				return 1;
			}
			return a.reviewerFullName.localeCompare(b.reviewerFullName);
		});
	});
	 ...
	 * Called by PkpOpenReviews component after mount
	 */
