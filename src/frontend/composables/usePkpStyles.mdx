...
2. **Global styles** - Registered via `usePkpVueComponentStyles()`
...
Themes or plugins can register Tailwind/utility classes for Vue components globally, via `usePkpVueComponentStyles()` (available as `pkp.modules.usePkpVueComponentStyles`). These styles only apply to Vue components styled with `usePkpStyles`:

```javascript
const {usePkpVueComponentStyles} = pkp.modules.usePkpVueComponentStyles;
...
Repeated calls are merged per component, element and nested component key. Register the styles before the Vue apps mount on `DOMContentLoaded`, for example from a script at the end of `<body>`.

Blade templates can use the `@vueComponentStyles` directive, which calls `TemplateManager::addVueComponentStyles()`. It outputs nothing where it is used, so it can sit next to the markup it styles, including inside a `data-vue-root` element:

```blade
@vueComponentStyles([
    'PkpDropdownMenu' => ['trigger' => 'bg-blue-500 px-4 py-2'],
])
```

The collected styles are output by the theme's footer, after the frontend scripts load and before the Vue apps mount. Every theme footer must include the tag right after loading the scripts. When styles were added but the tag is missing, a warning is logged.

```blade
@loadScript(['context' => 'frontend'])
@loadVueComponentStyles
```

In Smarty templates the tag is `{load_vue_component_styles}`.

Global styles can also contain nested component keys. They apply, at the lowest priority, to those components within the styled component, like nested keys of the `styles` prop. For example, `{CrossrefCitedByBody: {PkpButton: {root: 'btn'}}}` styles only the buttons inside `CrossrefCitedByBody`.
...
1. **Global styles** (`usePkpVueComponentStyles()`, including nested component keys)
