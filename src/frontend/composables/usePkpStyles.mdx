...
The `usePkpStyles` composable automatically handles parent-child relationships:

- **Pkp\* components** (e.g., `PkpComments`) create a styling context
- **Base\* components** (e.g., `BaseCommentsNew`) inherit their parent's context

This means Base components generate classes using their Pkp parent's name:

```html
<!-- Inside PkpComments -->
<BaseCommentsNew />

<!-- BaseCommentsNew uses cn('new') which generates: -->
<div class="PkpComments__new"></div>
```
...
1. **Separate structural and visual styles**

   - Put structural styles (display, flex, position) in component `<style>` blocks
   - Put visual styles (colors, padding, shadows) in theme LESS files

2. **Use meaningful element names**

   - Good: `trigger`, `triggerLabel`, `actionButtons`
   - Avoid: `div1`, `wrapper`, `inner`

3. **Document available style keys**

   - List customizable elements in component documentation
   - Show which elements support global/local styling

4. **Use override mode sparingly**
   - Prefer extending styles over replacing them
   - Only use `!` prefix when you need complete control
