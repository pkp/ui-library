<template>
	...
	>
		...
		<PkpTable :labelled-by="labelId" :described-by="descriptionId">
			<TableHeader>
				<TableColumn>{{ t('submission.funders.funder.grant.doi') }}</TableColumn>
				<TableColumn>{{ t('submission.funders.funder.grant.number') }}</TableColumn>
				<TableColumn>{{ t('submission.funders.funder.grant.name') }}</TableColumn>
				<TableColumn class="w-[100px]">&nbsp;</TableColumn>
			</TableHeader>
			<TableBody>
				<TableRow v-for="(row, index) in currentValue" :key="index">
					<TableCell>
						<FieldText
							name="grantDoi"
							:value="row.grantDoi"
							:all-errors="{grantDoi: errors?.[index]?.grantDoi}"
							@change="
