...
Genre options should be filtered to only include dependent genres (e.g., Image, Multimedia, HTML Stylesheet). Each genre option includes a `genreSupportsHighRes` flag that controls whether the resolution type dropdown is enabled.
...
<script setup>
const genreOptions = [
	{value: 9, label: 'Multimedia', genreSupportsHighRes: true},
	{value: 10, label: 'Image', genreSupportsHighRes: true},
	{value: 11, label: 'HTML Stylesheet', genreSupportsHighRes: false},
];
