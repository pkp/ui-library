							...
							<div class="ml-8 mr-8 flex-grow" data-cy="sidemodal-header">
								<div class="flex">
									<div class="flex-grow">
										...
										<DialogTitle
											as="h1"
											class="mt-1 text-4xl-bold text-heading"
										>
...
import {focusFirst} from './modalHelpers';
...
function handleAutoFocus(event) {
	if (container.value) {
		focusFirst(container.value, event);
	}
}
</script>
<style scoped>
...
.DialogContent[data-state='open'] {
	animation: sideModalSlideIn 400ms ease-in-out;
}
.DialogContent[data-state='closed'] {
	animation: sideModalSlideOut 400ms ease-in-out;
}
...
html[dir='rtl'] .DialogContent[data-state='open'] {
	animation: sideModalSlideInRtl 400ms cubic-bezier(0.4, 0, 0.2, 1);
}
html[dir='rtl'] .DialogContent[data-state='closed'] {
	animation: sideModalSlideOutRtl 400ms cubic-bezier(0.4, 0, 0.2, 1);
}
</style>
