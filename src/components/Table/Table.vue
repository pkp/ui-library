...
const registerRowHeader = (rowHeaderId) => {
	if (rowHeaderId) {
		rowHeaderIds.value.push(rowHeaderId);
	}
};

const unregisterRowHeader = (rowHeaderId) => {
	if (rowHeaderId) {
		const index = rowHeaderIds.value.indexOf(rowHeaderId);
		if (index !== -1) {
			rowHeaderIds.value.splice(index, 1);
		}
	}
};
